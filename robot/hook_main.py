...
def show_handle_image(image_path: str):
    """Affiche l'image de la poignée détectée"""
    if not Path(image_path).exists():
        print(f"[WARN] Image poignée introuvable : {image_path}")
        return
    
    img = cv2.imread(image_path)
    if img is not None:
        cv2.imshow("Poignée détectée - Hook Grasp", img)
        cv2.waitKey(0)  # Non-bloquant
        print(f"[INFO] Affichage de l'image poignée : {Path(image_path).name}")

def main():
    parser = argparse.ArgumentParser(description="Hook grasp sequence")
    parser.add_argument("ply_file", help="Fichier PLY (non utilisé)")
    parser.add_argument("--handle_image", help="Image de la poignée détectée")
    args = parser.parse_args()
    
    # Afficher l'image de la poignée si fournie
