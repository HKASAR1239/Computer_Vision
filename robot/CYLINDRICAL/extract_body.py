...
Aligns a partial cylindrical point cloud using PCA, centers it by Z-midpoint (with configurable offset), and extracts a configurable slice around the midpoint along the Z-axis.

Usage:
    python extract_body.py input.ply
    python extract_body.py input.ply -o body_slice.pcd

Configuration:
    SLICE_HALF_HEIGHT: half the slice thickness in meters (e.g. 0.01 = 1 cm)
    PROP_Z_OFFSET: proportional offset to apply to the Z-midpoint (fraction of total height). Positive values lower the midpoint.

Original file: extract_body.py citeturn0file0
"""
