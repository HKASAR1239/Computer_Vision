#!/usr/bin/env python3
"""
fit_and_superpose_cylinder_dynamic.py

Fits an infinite cylinder to a partial point cloud by optimizing axis orientation,
center, and radius so the cylinder body best follows the data.
Uses multiple PCA-based initializations to avoid local minima.
Visualizes the fit overlaid on the cloud and optionally saves outputs.

Usage:
    python fit_and_superpose_cylinder_dynamic.py input.ply \
        [--save-cylinder cyl.ply] [--save-merged merged.ply] [--samples N]
...
def generate_initial_guesses(points):
    ...
    for idx in range(3):
        ...
        # approximate radius as mean distance to axis
...
def fit_cylinder(points):
    # Try multiple PCA-based starts to avoid poor local minima
    best_cost = np.inf
    best_sol = None
...
def main():
    ...
    pcd.paint_uniform_color([0.6,0.6,0.6])
    cyl_mesh.paint_uniform_color([1,0,0])
    o3d.visualization.draw_geometries([pcd, cyl_mesh], width=1024, height=768)
