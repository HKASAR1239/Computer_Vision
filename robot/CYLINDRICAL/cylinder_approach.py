import cv2
import numpy as np
import sys
import time
import pyrealsense2 as rs

# --- PARAMÈTRES DE CALIBRATION (à adapter) ---
# Matrice de la caméra
camera_matrix = np.array([[800,   0, 320],
                          [  0, 800, 240],
                          [  0,   0,   1]], dtype=np.float32)
# Coefficients de distorsion (mettre les vôtres si besoin)
dist_coeffs = np.zeros((5,1))

# Longueur du marker en mètres (à ajuster selon votre ArUco)
marker_length = 0.05

# 3D coordinates of marker corners in marker frame (object points)
# Order: top-left, top-right, bottom-right, bottom-left
objp = np.array([
    [-marker_length/2,  marker_length/2, 0],
    [ marker_length/2,  marker_length/2, 0],
    [ marker_length/2, -marker_length/2, 0],
    [-marker_length/2, -marker_length/2, 0]
], dtype=np.float32)

def main(duration=5.0, realsense_pipeline=None):
    """
    Fonction principale - utilise la méthode depth qui fonctionne correctement
    """
    # Initialisation du détecteur ArUco - API moderne OpenCV
    aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_50)
    parameters = cv2.aruco.DetectorParameters()

    # Use RealSense if provided, otherwise fallback to default camera
    use_realsense = realsense_pipeline is not None
    if not use_realsense:
        cap = cv2.VideoCapture(0)

    approach = None

    # Timer configurable (5 secondes par défaut)
    start_time = time.time()

    print(f"Détection ArUco en cours pour {duration} secondes...", file=sys.stderr)

    while time.time() - start_time < duration:
        if use_realsense:
            # Get frame from RealSense
            frames = realsense_pipeline.wait_for_frames()
            color_frame = frames.get_color_frame()
            if not color_frame:
                continue
            frame = np.asanyarray(color_frame.get_data())
        else:
            # Get frame from regular camera
            ret, frame = cap.read()
            if not ret:
                print("Erreur: Impossible d'ouvrir la caméra", file=sys.stderr)
                break

        # Détection des marqueurs - API moderne
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        corners, ids, _ = cv2.aruco.detectMarkers(gray, aruco_dict, parameters=parameters)

        if ids is not None and len(ids) >= 2:
            # Recherche des deux marqueurs d'intérêt (0 et 1)
            ids = ids.flatten()
            if 0 in ids and 1 in ids:
                i0 = list(ids).index(0)
                i1 = list(ids).index(1)
                c0 = corners[i0][0]
                c1 = corners[i1][0]

                # Calcul de la pose
                _, r0, t0 = cv2.solvePnP(objp, c0.astype(np.float32), camera_matrix, dist_coeffs)
                _, r1, t1 = cv2.solvePnP(objp, c1.astype(np.float32), camera_matrix, dist_coeffs)

                # Méthode depth : Compare la profondeur (Z) des deux markers
                # Si marker 1 est plus proche de la caméra (Z plus petit) -> "top"
                # Si marker 1 est plus loin de la caméra (Z plus grand) -> "body"
                depth_diff = float(t1[2]) - float(t0[2])
                approach = "top" if depth_diff > 0 else "body"

                # Debug simple et clair
                debug = f"approach = {approach} (depth_diff={depth_diff:.3f})"
                print(debug, file=sys.stderr)

                # Affichage sur l'image
                c0_center = tuple(c0.mean(axis=0).astype(int))
                c1_center = tuple(c1.mean(axis=0).astype(int))
                cv2.line(frame, c0_center, c1_center, (0,255,0), 2)
                cv2.circle(frame, c0_center, 5, (255,0,0), -1)
                cv2.circle(frame, c1_center, 5, (0,0,255), -1)
                cv2.putText(frame, debug, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)
                cv2.putText(frame, f"Marker 0 (bleu): depth={float(t0[2]):.3f}", (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1)
                cv2.putText(frame, f"Marker 1 (rouge): depth={float(t1[2]):.3f}", (10,80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                cv2.drawFrameAxes(frame, camera_matrix, dist_coeffs, r0, t0, marker_length*0.5)

        # Afficher le temps restant
        remaining_time = duration - (time.time() - start_time)
        cv2.putText(frame, f"Temps restant: {remaining_time:.1f}s", (10, 110), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

        cv2.imshow("ArUco Top/Body Detection", frame)
        cv2.waitKey(1)

    # Libération des ressources
    if not use_realsense:
        cap.release()
    cv2.destroyAllWindows()

    # Sortie finale attendue par main.py: un seul mot sur stdout
    if approach is not None:
        print(approach)
    else:
        print("unknown")

# Backward compatibility: if called directly, use default camera
if __name__ == "__main__":
    main()