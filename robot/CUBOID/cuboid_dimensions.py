...
Cela évite d’obtenir des inversions comme Width > Length sur certains
nuages.

Principales améliorations :
• Nettoyage statistique du nuage pour supprimer le bruit.
• Détection de plans RANSAC multipasse avec élimination de doublons.
• Complément d'axes manquants par PCA (analyse en composantes principales).
• Orthonormalisation de Gram–Schmidt pour garantir des axes perpendiculaires.
• Estimation robuste des dimensions par quantiles (1 %–99 %) pour
  ignorer les points aberrants.
• Option --voxel pour contrôler un échantillonnage voxel (accélère + stabilise).
• Sortie métrique (mètres) + possibilité d’afficher l’OBB.

Usage :
    python extract_dims_robust.py <cloud.[ply|pcd]> [--visualize] [--voxel 0.003]
...
def segment_planes(pcd: o3d.geometry.PointCloud,
    ...
    """Renvoie un ensemble de normales de plans suffisamment distinctes."""
    ...
    for _ in range(max_planes):
        ...
        n /= np.linalg.norm(n)
        # ajoute si différente des précédentes
...
def extract_dimensions(pcd: o3d.geometry.PointCloud,
    ...
    if "width" in dims_named and "length" in dims_named and dims_named["width"] > dims_named["length"]:
        ...
        axes[1], axes[2] = axes[2], axes[1]  # maintient la correspondance axe‑dimension
...
def main() -> None:
    ...
    parser.add_argument("--visualize", action="store_true", help="affiche la boîte ajustée")
