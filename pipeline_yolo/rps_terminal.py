...
print(f"Caméra initialisée : {cap.get(cv2.CAP_PROP_FRAME_WIDTH)}×{cap.get(cv2.CAP_PROP_FRAME_HEIGHT)}")
...
# --------------------- Détection du geste (logic ONLY) --------------

def detect_gesture(lm_obj):
    """Retourne ROCK, PAPER ou SCISSORS depuis les landmarks Mediapipe."""
    pts = lm_obj.landmark
    wrist = pts[0]
    thumb_tip = pts[4]
...
print("\nRPS Terminal — robot 0,5 s avant votre geste")
print("--------------------------------------------------")
print("Paume ouverte : start 3-2-1 ; robot bouge à 2,5 s (0,5 s avant capture)")
print("'d' détection directe | 'q' quitter | 's' compte à rebours manuel")
print("--------------------------------------------------")
...
try:
    while True:
        ...
        # -------- WAIT / SHOW : détection temps réel --------
        if mode != MODE_COUNT:
            ...
            if res.multi_hand_landmarks:
                ...
                for lm in res.multi_hand_landmarks:
                    ...
                    if mode == MODE_WAIT:
                        ...
                        label = lbl
                        cv2.putText(frame, f"Détection : {lbl}", (10, 60),
        ...
        if mode == MODE_WAIT and trigger_counter >= AUTO_TRIGGER_FRAMES:
            ...
            trigger_counter = 0
            print("\nPaume + rock détectée — lancement compte à rebours !")
            ...
            if idx_num < 3:
                ...
                if not move_executed and elapsed >= ROBOT_LAUNCH_T and ai_move in idx:
                    try:
                        ...
                        print(f"\n>>> ROBOT lancé à {elapsed:.2f}s : {robot_move}")
                ...
                # Capture robuste : jusqu’à 5 frames pour éviter NO HAND
                gesture_found = False
                for _ in range(5):
        ...
        if key == ord('s') and mode == MODE_WAIT:
            ...
            trigger_counter = 0
            print("\nDépart manuel compte à rebours (touche 's')")
        if key == ord('d') and mode == MODE_WAIT:
            ...
            if res.multi_hand_landmarks:
                ...
                label = detect_gesture(lm)
                ai_move = recommend(label)
                print(f"Geste : {label}  |  IA propose : {ai_move}")
                if ai_move in idx:
                    try:
                        robot_move = ai_move.lower()
