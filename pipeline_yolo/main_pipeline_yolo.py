...
def get_best_device():
    ...
    if torch.backends.mps.is_available():
        print("[Device] MPS (Metal Performance Shaders) détecté et utilisé")
        return torch.device("mps")
    elif torch.cuda.is_available():
        print(f"[Device] CUDA détecté - GPU: {torch.cuda.get_device_name()}")
        return torch.device("cuda")
    else:
        print("[Device] CPU utilisé")
        return torch.device("cpu")

# Device global pour tous les modèles
...
SHAPE2SCRIPT = {
    "spherical": "torque_spherical.py",
    "cylinder":  "torque_cylinder.py",
    "cuboid":    "torque_cuboid.py",
}
...
class VoiceController(threading.Thread):
    def __init__(self):
        ...
        print("[Audio] Chargement des modèles…")
        # WakeNet
        self.wakenet = WakeNet().to(DEVICE)
        self.wakenet.load_state_dict(torch.load("models/wakenet.pt", map_location=DEVICE))
        self.wakenet.eval()
        # CmdNet
        self.idx2cmd = json.loads(Path("models/idx2cmd.json").read_text())
        self.cmdnet = CmdNet(len(self.idx2cmd)).to(DEVICE)
        self.cmdnet.load_state_dict(torch.load("models/cmdnet.pt", map_location=DEVICE))
        self.cmdnet.eval()
        print(f"[Audio] Modèles chargés sur {DEVICE}")
        # Audio stream
    ...
    def _exec(self, cmd: str):
        ...
        try:
            if cmd == "GRAB":
                grab_pending = True
                grab_immediate = True
                print("[Audio] GRAB → préhension immédiate!")
            elif cmd == "DROP":
                subprocess.run(["python", "reset.py"])
            elif cmd == "YOLO":
                subprocess.run(["python", "HAND_grasp.py", "--movement", "yolo"])
            elif cmd == "LUMBAR":
                subprocess.run(["python", "lumbar.py"])
                print("[Audio] LUMBAR → exécution lumbar.py")
            elif cmd == "EAT":
                subprocess.run(["python", "eat.py"])
                print("[Audio] EAT → exécution eat.py")
            elif cmd == "CHESS":
                subprocess.run(["python", "chess.py"])
                print("[Audio] CHESS → exécution chess.py")
            elif cmd == "START RPS":
                if rps_process is None or rps_process.poll() is not None:
                    rps_process = subprocess.Popen(["python", "rps_terminal.py"])
                else:
                    print("[Audio] RPS déjà actif.")
            elif cmd == "STOP RPS":
                if rps_process and rps_process.poll() is None:
                    rps_process.terminate(); print("[Audio] RPS arrêté.")
            else:
                print(f"[Audio] Commande inconnue : {cmd}")
        except Exception as e:
            print(f"[Audio] Erreur action : {e}")

    def run(self):
        global freeze_countdown
        print("👂 Dites le wake-word…")
        ...
        with self.stream:
            ...
            while True:
                ...
                if self._vad(ring[-int(0.3*SAMPLE_RATE):]) and self._wake(ring):
                    ...
                    print("⚡ Wake-word détecté – dites votre commande…")
                    cmd = None
                    while cmd is None:
                        cmd = self._recognise_cmd()
                        if cmd is None:
                            print("🤔 Commande inconnue, répétez…")
                    print(f"🗣️  → « {cmd} »")
                    self._exec(cmd)
...
class ObjectSelector:
    def __init__(self, cam_idx=0, model_path="yolov8m.pt"):
        ...
        if not self.cap.isOpened(): raise RuntimeError("Caméra non dispo.")
        self.model = YOLO(model_path)
        if DEVICE.type in ["mps","cuda"]:
            print(f"[Vision] YOLO configuré pour utiliser {DEVICE}")
        else:
            print("[Vision] YOLO utilise le CPU")
    ...
    def loop(self):
        ...
        while True:
            ...
            cv2.putText(frame, f"Objets valides: {len(boxes)}", (10,120), FONT,0.6,(255,255,255),1)
            # LOGIQUE GRAB
            if grab_pending:
                if label:
                    ...
                    if shape:
                        if grab_immediate:
                            print(f"[Vision] Préhension immédiate → {shape}")
                            ...
                            if not freeze_countdown:
                                ...
                                cv2.putText(frame, "En attente commande vocale...", (10,150), FONT,0.8,(0,0,255),2)
                    else:
                        cv2.putText(frame, "Objet non manipulable!", (10,150), FONT,0.8,(0,0,255),2)
                        grab_pending = grab_immediate = False; self.prev_shape = self.stable_since = None
                else:
                    cv2.putText(frame, "Aucun objet sélectionné!", (10,150), FONT,0.8,(0,0,255),2)
                    grab_pending = grab_immediate = False; self.prev_shape = self.stable_since = None
            # LOGIQUE AUTO
                ...
                if shape:
                    if not freeze_countdown:
                        if shape == self.prev_shape and self.auto_stable_since and time.time()-self.auto_stable_since >= 5:
                            print(f"[Vision] AUTO: {shape} stable 5s → grasp auto")
                            subprocess.Popen(["python", SHAPE2SCRIPT[shape]])
                        ...
                        cv2.putText(frame, "En attente commande vocale...", (10,150), FONT,0.7,(0,255,0),2)
                else:
                    self.prev_shape = self.auto_stable_since = None
            # Reset si rien
            else:
                self.prev_shape = self.stable_since = self.auto_stable_since = None
            # Touches debug et commandes
            key = cv2.waitKey(1) & 0xFF
            if key==ord('g'): grab_pending=True; grab_immediate=True; print("[Debug] g → GRAB immédiat…")
            elif key==ord('d'): subprocess.run(["python","HAND_grasp.py","--movement","open"]); print("[Debug] d → DROP")
            elif key==ord('y'): subprocess.run(["python","HAND_grasp.py","--movement","yolo"]); print("[Debug] y → YOLO")
                ...
                if rps_process is None or rps_process.poll() is not None: rps_process = subprocess.Popen(["python","rps_terminal.py"]); print("[Debug] s → START RPS")
                else: print("[Debug] RPS déjà actif")
            elif key==ord('p'):  # STOP RPS
                if rps_process and rps_process.poll() is None: rps_process.terminate(); print("[Debug] p → STOP RPS")
            elif key==ord('c'): subprocess.run(["python","chess.py"]); print("[Debug] c → CHESS")
            elif key==ord('l'): subprocess.run(["python","lumbar.py"]); print("[Debug] l → LUMBAR")
            elif key==ord('e'): subprocess.run(["python","eat.py"]); print("[Debug] e → EAT")
...
# ────────────────────────── MAIN ───────────────────────────
def main():
    print(f"🚀 Démarrage du pipeline avec {DEVICE}")
