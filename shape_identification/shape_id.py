...
"""
shape_id.py – V9
Détecte  spherique | cylindrique | tablet | cuboid
Robuste partiels / creux / solides
"""
import os, sys, json, struct, numpy as np

# ────────────────── I/O (.ply binaire ou JSON) ──────────────────────────────
def load_points(path: str) -> np.ndarray:
    ...
    with open(path, "rb") as f:
        ...
        while not line.startswith(b"end_header"):
            line = f.readline()
            if not line:
                raise ValueError("PLY sans end_header")
            header.append(line)
        header = b"".join(header).decode(errors="ignore")

        # nb de sommets et liste des types x,y,z
...
def classify(points, *,
             sph_err_max=0.07,
             sph_iso_max=1.8,        # λ1/λ2 maxi pour sphère
             tube_cv_max=0.50,
             cross_max=4.0,          # λ2/λ3 maxi pour section ≈ circulaire
             tablet_max_h2r=0.6,
             debug=False):
    # PCA et métriques communes
    (l1, l2, l3), vecs, pts_c = pca(points)
    r12 = l1 / (l2 + 1e-12)
    r23 = l2 / (l3 + 1e-12)

    # 1. Sphère – faible erreur ET quasi-isotropie des deux axes principaux
    err = sphere_err(points)
    if debug:
        print(f"[sphère] err={err:.3f}  r12={r12:.2f}")
    if err < sph_err_max and r12 < sph_iso_max:
        return "spherique"

    # 2. Cylindre / Tablet – rayon régulier ET section ~ circulaire
